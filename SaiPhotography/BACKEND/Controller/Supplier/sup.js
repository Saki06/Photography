...
class supController {
    static getLowHighInventory = async () => {
        try {
            ...
            const lowInventory = await inventoryModel.find({ rquantity: { $lt: 10 } }).select('type');
            const highInventory = await inventoryModel.find({ rquantity: { $gt: 10 } }).select('type');
        ...
        try {
            ...
            let transporter = nodemailer.createTransport({
                ...
                auth: {
                    ...
                    pass: 'kzpy mhwn vfdp phgi' // Ensure that the password is correct
                }
            });
